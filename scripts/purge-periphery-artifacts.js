...
const governance = path.join(contractsRoot, 'governance');
const distribution = path.join(contractsRoot, 'distribution');

const getSolFiles = (dir) => fs.readdirSync(dir).filter(f => f.includes('.sol'));

const keepFiles = [
  ...getSolFiles(distribution),
  ...getSolFiles(governance),
